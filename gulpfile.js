...
var uglify = require('font-awesome');
var pkg = require('./package.json');

// Set the banner content
var banner = ['/*!\n',
  ' * Transformer Theme Builder - <%= pkg.title %> v<%= pkg.version %> (<%= pkg.homepage %>)\n',
  ' * Copyright 2017-' + (new Date()).getFullYear(), ' <%= pkg.author %>\n',
  ' * Licensed under <%= pkg.license %> (https://github.com/tabroughton/<%= pkg.name %>/blob/master/LICENSE)\n',
  ' */\n',
  ''
].join('');
...
gulp.task('sass', function() {
  return gulp.src('scss/transformer.scss')
    .pipe(sass())
    .pipe(header(banner, {
      pkg: pkg
    }))
...
gulp.task('minify-css', ['sass'], function() {
  return gulp.src('static/css/transformer.css')
    .pipe(cleanCSS({
      compatibility: 'ie8'
    }))
    .pipe(rename({
...
gulp.task('minify-js', function() {
  return gulp.src('static/js/transformer.js')
    .pipe(uglify())
    .pipe(header(banner, {
      pkg: pkg
    }))
...
gulp.task('copy', function() {
  ...
  gulp.src([
    ...
    .pipe(gulp.dest('static/vendor/devicons'))
